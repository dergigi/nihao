...
## Bugs / DX
- [ ] Unknown flags silently ignored — `--nsec` is a natural guess for `--sec`, should error or warn on unknown flags
...
- [ ] `--sec` → `--nsec` alias — add `--nsec` as an alias since it's the more intuitive flag name
